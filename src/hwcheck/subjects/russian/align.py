...
_LETTER = re.compile(r"[а-яa-z]")
# знаки, которые OCR приклеивает к слову: в вопрос ребёнку и в находку они не идут. Дефис — тоже
# край слова («прово-»); внутри слова он остаётся — так ребёнок и написал («кто-то», «сде-делал»)
_EDGE_PUNCTUATION = ".,!?;:«»\"'()[]{}-‐–—…"


def normalize(word: str) -> str:
    # дефис при сверке не значим: на краю слова это перенос («сред-») или тире, которое OCR
    # приклеил к соседу («до дома-»), внутри — след переноса в склеенном слове («сде-делал» —
    # это «сделал» с ошибкой ребёнка). Эталон сверяется так же, поэтому «кто-то» сойдётся
    return _STRIP.sub("", word.lower().replace("ё", "е"))


def is_word(text: str) -> bool:
    """Слово ли это. Одинокий знак («!») и номер пункта («2)») — не слово: в эталоне знаков и
    цифр нет вовсе (`gaps.tokenize`), сверять их не с чем, а в находках это шум."""
    return bool(_LETTER.search(normalize(text)))


def display_word(text: str) -> str:
    """Слово как его показывают ребёнку: «здесь написано «спасти»?», а не «спасти,»?.

    Само прочитанное слово (`Word.text`) не меняем — по нему кропается картинка и видно, что
    именно прочитал OCR. Если от слова после обрезки ничего не остаётся (прочитан один знак),
    показываем как есть — иначе вопрос будет про пустоту.
    """
    return text.strip(_EDGE_PUNCTUATION) or text
