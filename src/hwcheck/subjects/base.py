...
class SubjectPage(BaseModel):
    ...
    # на сколько градусов по часовой стрелке модуль довернул фото, чтобы распознать страницу:
    # координаты слов относятся к повёрнутому кадру, поэтому бот кладёт в хранилище его же —
    # иначе кроп в вопросе «здесь написано …?» покажет не то место (живой прогон 18.09)
    rotation: int = 0
