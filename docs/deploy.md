...
стадии контейнер на VPS не запускается** — только собирается, чтобы образ был готов к моменту
подключения реального движка (`ReadingPipelineEngine`, этап «русский»).
...
Лимит памяти — `3 ГБ` (не `1,5 ГБ`, как в исходных ограничениях): по спайку
`docs/research/2026-09-17-readingpipeline-vps.md` пайплайн ReadingPipeline на реальных фото стабильно
занимает ~2,3–2,5 ГБ и пиково до ~3,06 ГБ, `1,5 ГБ` недостижим. На общем VPS свободно ~3,7–3,8 ГБ —
запускать OCR одновременно с ботом можно только проверив `free -m` заранее, запас на конкурентный
плотный запрос не гарантирован. Движок по умолчанию — `OCR_ENGINE=fake` (переменная в `.env`), без
портов наружу: бот ходит по compose-сети как к `http://ocr:8080` (`OCR_URL` в `environment` бота).
