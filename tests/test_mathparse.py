...
PARSEABLE_CASES = [
    ...
    # юникодный минус
    ("−5+6=1", ["1", "1"]),
    # деление дробей «:» (живые логи 13.09): дробная черта связывает сильнее двоеточия,
    # «4/5 : 9/10» — это (4/5):(9/10), а не 4/5/9/10 = 2/225
    ("4/5 : 9/10 = 4/5 * 10/9 = 40/45 = 8/9", ["8/9", "8/9", "8/9", "8/9"]),
    ("2 : 1/2 = 4", ["4", "4"]),
    ("1 1/2 : 3/4 = 2", ["2", "2"]),
    ("3/4 : 3 = 1/4", ["1/4", "1/4"]),
    ("-3/4 : 1/2 = -3/2", ["-3/2", "-3/2"]),
    ("1,5/3 : 1/4 = 2", ["2", "2"]),
    # обычный приоритет там, где двоеточия нет, не меняется
    ("12/6/2 = 1", ["1", "1"]),
    ("2^3/4 = 2", ["2", "2"]),
    ("4/2^3 = 0,5", ["1/2", "1/2"]),  # знаменатель в степени: 4/(2^3), а не (4/2)^3
    ("120 + 320/4 = 200", ["200", "200"]),
]
...
@pytest.mark.parametrize("line", ["(120 + 320) : 4 : 2 = 40 . 40", "40 . 40 = 1600"])
def test_sympy_internal_errors_mean_not_checkable(line: str) -> None:
    # живые логи 06.09: parse_expr на «40 . 40» бросал AttributeError, и падала
    # обработка всего фото вместо «строка не проверяется»
    assert parse_line(line) is None
