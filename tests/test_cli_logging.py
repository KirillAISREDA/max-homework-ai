...
def test_bot_log_goes_to_rotating_file(tmp_path: Path) -> None:
    ...
    try:
        configure_bot_logging(Settings(_env_file=None, log_path=str(log_path), environment="dev"))
        ...
        # ревью: забытый ENVIRONMENT=dev пишет транскрипции тетрадей в файл — предупредить
